...
<template>
    <router-link
v-if="link && link.startsWith('/')" class="btn" :class="{
        'icon-only': iconOnly,
        'btn-large': large,
...
v-else class="btn" :class="{
        ...
        'btn-outline': outline,
        'color-accent-contrast': accentedButton,
    }" :disabled="disabled" @click="action">
        <slot />
        <UnknownIcon v-if="!$slots.default" />
    </button>
</template>
