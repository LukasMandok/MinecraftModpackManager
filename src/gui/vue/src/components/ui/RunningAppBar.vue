<template>
    <div class="action-groups">
        ...
        <!-- Button that indicates, if something is downloaded at the moment, allows to open info panel-->
        <Button 
            v-if="currentLoadingBars.length > 0" ref="infoButton" icon-only class="icon-button show-card-icon"
            @click="toggleCard()">
            <DownloadIcon />
        </Button>
        ...
        <!-- Current Mincraft Profile Selected -->
        <div v-if="selectedProfile" class="status">
            <!-- Indication if instance is running -->
            <span class="circle running" />
            <div ref="profileButton" class="running-text">
                <!-- Router to currently selected Instance -->
                <router-link :to="`/instance/${encodeURIComponent(selectedProfile.path)}`">
                    {{ selectedProfile.metadata.name }}
                </router-link>
                <!-- Options to select other directories with dropdown menu -->
                <div 
                    v-if="currentProcesses.length > 1" class="arrow button-base" :class="{ rotate: showProfiles }"
    ...
    <transition name="download">
        <Card v-if="showProfiles === true && currentProcesses.length > 0" ref="profiles" class="profile-card">
            <Button 
                v-for="profile in currentProcesses" :key="profile.id" class="profile-button"
                @click="selectProfile(profile)">
                <div class="text"><span class="circle running" /> {{ profile.metadata.name }}</div>
                <Button v-tooltip="'Stop instance'" icon-only class="icon-button stop" @click.stop="stop(profile.path)">
                    <StopCircleIcon />
                </Button>
                <Button v-tooltip="'View logs'" icon-only class="icon-button" @click.stop="goToTerminal(profile.path)">
                    <TerminalSquareIcon />
                </Button>
            </Button>
        </Card>
...
<script setup>
    ...
    // TDOO: get list of profiles from python backend
    // const profiles = ref([])
